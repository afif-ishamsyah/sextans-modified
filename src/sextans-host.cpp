...
void sparesifyCSC(float *A, int m , int k, int lda, vector<float>& Val, vector<int>& RowIndex, vector<int>& ColumnPtr, int& nnz);

extern "C" 
{
    void sextans_fgemm(int transposeA, int transposeB, int m, int n, int k, float alpha, float *a, int lda, float *b, int ldb, float beta, float *c, int ldc )
    {
        printf("start host\n");
        ...
        std::string bitstream = "Sextans_xilinx_u280_xdma_201920_3.xclbin";
...
void sparesifyCSC(float *A, int m , int k, int lda, vector<float>& Val, vector<int>& RowIndex, vector<int>& ColumnPtr, int& nnz)
{
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < k; j++) {
            if (A[i + lda * j] != 0) {
                Val.push_back(A[i + lda * j]);
                RowIndex.push_back(j);
 
                nnz++;
            }
        }
        ColumnPtr.push_back(nnz);
    }
}
